...
import Component_index from "../component_index";

export default function Service() {
  const servies_detail = [
    {
      service_img: Image_index.services_img[0],
...
designed to help you achieve your aquatic goals.`,
  ...
  const change_service = (event) => {
    const selected = servies_detail.find(
      (s) => s.service_heading == event.target.value
    );
  ...
  return (
    <>
    {Component_index.Navbar}
      <div className={servicestyle.heading}>
        <h1>Services</h1>
        <p>
          At MH Fitness Club, we're committed to helping you achieve your
          fitness goals and lead a healthier lifestyle. Our comprehensive range
          of services is designed to cater to your unique needs and preferences.
          Whether you're a seasoned athlete or just starting your fitness
          journey, we have something for everyone. Here's a glimpse of what we
          offer:
        </p>
      </div>
      <div className={servicestyle.main}>
        <div className={servicestyle.menu}>
          <div className={servicestyle.menu1}>
            <li onClick={() => service_Set((service = servies_detail[0]))}>
              Cardio
            </li>
            <li onClick={() => service_Set((service = servies_detail[2]))}>
              Diet
            </li>
            <li onClick={() => service_Set((service = servies_detail[1]))}>
              Gym
            </li>
            <li onClick={() => service_Set((service = servies_detail[3]))}>
              Swim
            </li>
          </div>
